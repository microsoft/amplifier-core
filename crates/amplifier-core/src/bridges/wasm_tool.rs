...
///
/// Applies the given [`WasmLimits`](super::WasmLimits):
/// - **C-02**: CPU time limit via epoch interruption deadline
/// - **C-02**: Memory limit via [`StoreLimitsBuilder`](wasmtime::StoreLimitsBuilder)
/// - **H-01**: Restricted WASI capabilities — null I/O, no inherited env/args
pub(crate) fn create_linker_and_store(
    ...
    let wasi = wasmtime_wasi::WasiCtxBuilder::new()
        .stdin(wasmtime_wasi::p2::pipe::ClosedInputStream)
    ...
    // C-02: Wire up the limiter so the store enforces memory limits
...
impl Tool for WasmToolBridge {
    ...
    fn execute(
        ...
        Box::pin(async move {
            ...
            let result_bytes =
                tokio::task::spawn_blocking(move || call_execute(&engine, &component, input_bytes))
                    .await
...
mod tests {
    ...
    fn load_echo_tool_from_bytes() {
        ...
        let bridge = WasmToolBridge::from_bytes(&bytes, engine).expect("from_bytes should succeed");
