...
fn call_list_models(engine: &Engine, component: &Component) -> WasmResult<Vec<u8>> {
    ...
    let func = get_provider_func::<(), (Result<Vec<u8>, String>,)>(
        &instance,
        &mut store,
        "list-models",
    )?;
...
impl Provider for WasmProviderBridge {
    ...
    fn list_models(
        ...
        Box::pin(async move {
            ...
            let result_bytes = tokio::task::spawn_blocking(move || {
                call_list_models(&engine, &component)
            })
            .await
            .map_err(|e| {
    ...
    fn complete(
        ...
        Box::pin(async move {
            ...
            .await
            .map_err(|e| {
                wasm_provider_error(format!("WASM provider complete task panicked: {e}"))
            })?
...
mod tests {
        ...
        for p in &candidates {
            if p.exists() {
                return std::fs::read(p).unwrap_or_else(|e| {
                    panic!("Failed to read echo-provider.wasm at {p:?}: {e}")
                });
            }
        }
        panic!(
    ...
    fn echo_provider_get_info() {
        ...
        assert_eq!(info.id, "echo-provider", "expected info.id == 'echo-provider'");
        assert_eq!(
    ...
    async fn echo_provider_complete() {
        ...
        let response = bridge.complete(request).await.expect("complete should succeed");
