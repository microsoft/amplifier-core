...
type HandleFunc = wasmtime::component::TypedFunc<(Vec<u8>,), (Result<Vec<u8>, String>,)>;
...
/// Look up the `handle` typed function export from a component instance.
///
/// Tries:
/// 1. Direct root-level export `"handle"`
/// 2. Nested inside the [`INTERFACE_NAME`] exported instance
fn get_handle_func(
    instance: &wasmtime::component::Instance,
    store: &mut Store<WasmState>,
) -> WasmResult<HandleFunc> {
    // Try direct root-level export first.
    if let Ok(f) = instance
        .get_typed_func::<(Vec<u8>,), (Result<Vec<u8>, String>,)>(&mut *store, "handle")
    {
        return Ok(f);
    }
    ...
    let func_idx = instance
        .get_export_index(&mut *store, Some(&iface_idx), "handle")
        .ok_or_else(|| {
            format!("export function 'handle' not found in '{INTERFACE_NAME}'")
        })?;
    let func = instance
        .get_typed_func::<(Vec<u8>,), (Result<Vec<u8>, String>,)>(&mut *store, &func_idx)
        .map_err(|e| format!("typed func lookup failed for 'handle': {e}"))?;
    Ok(func)
}
...
impl HookHandler for WasmHookBridge {
    fn handle(
        ...
        Box::pin(async move {
            ...
            let envelope_bytes =
                serde_json::to_vec(&envelope).map_err(|e| HookError::Other {
...
mod tests {
    ...
    async fn deny_hook_returns_deny_action() {
        ...
        let bridge =
            WasmHookBridge::from_bytes(&bytes, engine).expect("from_bytes should succeed");
