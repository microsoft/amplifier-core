            ...
            move |_caller,
                ...
                let result = tokio::runtime::Handle::current().block_on(async {
                    ...
                    let chat_request: crate::messages::ChatRequest =
                        serde_json::from_value(request_val)
                            .map_err(|e| format!("complete-with-provider: bad ChatRequest: {e}"))?;
            ...
            move |_caller,
                ...
                let result: Result<(), String> = (|| {
                    ...
                    let name = req
                        .get("name")
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| "register-capability: missing 'name' field".to_string())?;
...
fn call_execute_sync(
    ...
    let (mut linker, mut store) = create_linker_and_store(engine, &super::WasmLimits::default())?;
...
impl Orchestrator for WasmOrchestratorBridge {
    ...
    fn execute(
        ...
        Box::pin(async move {
            ...
            let request_bytes = serde_json::to_vec(&serde_json::json!({"prompt": prompt}))
                .map_err(|e| {
...
mod tests {
    ...
    async fn passthrough_orchestrator_with_wasm_echo_tool() {
        ...
        let echo_bridge =
            super::super::wasm_tool::WasmToolBridge::from_bytes(&echo_bytes, Arc::clone(&engine))
                .expect("echo-tool bridge should load");
        coordinator.mount_tool("echo-tool", Arc::new(echo_bridge));

        let bridge =
            WasmOrchestratorBridge::from_bytes(&orch_bytes, Arc::clone(&engine), coordinator)
                .expect("from_bytes should succeed");
