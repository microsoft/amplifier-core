...
async fn provider_complete() {
    ...
    // Verify name.
    assert_eq!(provider.name(), "echo-provider", "provider.name() mismatch");
...
async fn orchestrator_calls_kernel() {
    ...
    let echo_tool =
        load_wasm_tool(&echo_bytes, Arc::clone(&engine)).expect("load echo-tool for coordinator");
...
fn calculator_tool_loads_and_resolves() {
    ...
    let bytes = std::fs::read(&path).unwrap_or_else(|e| {
        panic!(
            "calculator-tool.wasm not found at {}: {}",
            path.display(),
            e
        )
    });
    ...
    assert!(spec.description.is_some(), "spec.description should be set");
}
