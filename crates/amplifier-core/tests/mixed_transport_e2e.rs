...
async fn mixed_transport_session() {
    ...
    let native_provider = Arc::new(FakeProvider::new("fake-provider", "native-provider-response"));
    ...
    let _unregister_deny_hook = coordinator
        .hooks()
        .register("tool:before_execute", wasm_hook, 0, Some("wasm-deny-hook".to_string()));
...
fn coordinator_to_dict_reflects_mixed_modules() {
    ...
    let echo_bytes = fixture("echo-tool.wasm");
    let wasm_tool =
        load_wasm_tool(&echo_bytes, engine).expect("load_wasm_tool should succeed");
    ...
    let providers = dict["providers"].as_array().expect("providers must be array");
    assert!(
